...
export function Navbar() {
  ...
  // Where "My tasks" should link
  const myHref = user ? (user.role === "tasker" ? "/myoffers" : `/users/${user.id}/tasks`) : "/login";

  // ❌ Don’t put hooks inside || or &&.
  // Call hooks unconditionally, then combine results.
  const matchUsersTasks = useMatch("/users/:userId/tasks");
  const matchLegacyTasks = useMatch("/tasks/:user_id"); // if you still keep this route
  const matchTaskerMyOffers = useMatch("/myoffers");
  const matchTaskerArea = useMatch("/tasker/*");

  const myActive = Boolean(
    matchUsersTasks ||
    matchLegacyTasks ||
    matchTaskerMyOffers ||
    matchTaskerArea
